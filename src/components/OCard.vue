<template>
  <Card ref="OCardRef" id="o-card">
    ...
    <template #content>
        ...
        <i v-if="mode === 'default' && data.icon" ref="OCardIconModeDefaultRef" :class="`fad ${data.icon} icon__mode-default`" />
    </template>
    <template v-if="$slots.footer !== undefined" #footer>
      <div class="'o-card__footer mt-5'">
          <slot name="footer" />
      </div>
    </template>
  </Card>
...
export default {
  ...
  props: {
    ...
    mounted() {
      nextTick(() => {
          if (this.borderColor != null) this.$refs.OCardRef.$el.style.borderLeft = `5px solid var(--special-color-${this.borderColor})`;
          ...
          if (this.contentColor != null) this.$refs.OCardRef.$el.style.backgroundColor = `var(--special-color-${this.contentColor})`;
